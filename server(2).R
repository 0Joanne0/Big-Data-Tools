...
# Enlève les majuscules, les underscores et les espaces en trop.
...
# Définit les équivalences (ex: "gcp" = "google cloud").
...
# On normalise les clés pour être sûr que ça matche
...
# Sert de filet de sécurité pour attraper des variantes (ex: "python programming" -> "python").
...
# C'est la fonction à appeler pour nettoyer une compétence.
...
server <- function(input, output, session) {
  ...
  session$onFlushed(function() {
    ...
    # mp_title = mêmes choix que exp_q
    updateSelectizeInput(
      session, "mp_title",
      choices = SEARCH_CHOICES,
      server  = FALSE,
      options = list(
        ...
        create = FALSE, persist = FALSE, delimiter = ";",
        maxOptions = 200,
        plugins = list("remove_button", "restore_on_backspace")
      )
    )
    
    # mp_loc = mêmes choix que exp_loc
    updateSelectizeInput(
      session, "mp_loc",
      choices = LOC_CHOICES,
      server  = TRUE,
      options = list(
        placeholder = "Ville, département, code postal...",
        plugins = list("remove_button"),
        create = FALSE, persist = FALSE, openOnFocus = TRUE,
        maxOptions = 200
      )
    )
    ...
    if (has_col(jobs_df, "Sector")) {
      ...
      updateSelectizeInput(session, "exp_sector", choices = sort(unique(jobs_df$Category)), server = TRUE)
    }
